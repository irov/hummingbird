...
int main( int _argc, char * _argv[] )
{
    ...
    if( hb_sharedmemory_read( &sharedmemory_handle, &in, sizeof( hb_node_newproject_in_t ), HB_NULLPTR ) == 0 )
    {
        return 0;
    }

    hb_db_initialze( "hb_node_newproject", in.db_uri );
    ...
    hb_make_int64_value( "script_revision", ~0U, 0, &new_value );
    ...
    memcpy( out.puid, oid, 12 );

    hb_sharedmemory_rewind( &sharedmemory_handle );
    hb_sharedmemory_write( &sharedmemory_handle, &out, sizeof( hb_node_newproject_out_t ) );
