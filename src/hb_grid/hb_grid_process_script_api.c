...
hb_result_t hb_grid_process_script_api( hb_grid_process_handle_t * _process, const hb_grid_process_script_api_in_data_t * _in, hb_grid_process_script_api_out_data_t * const _out )
{
    ...
    if( hb_script_api_call( script_handle, _in->api, _in->method, _in->json_args, _out->response_data, HB_GRID_RESPONSE_DATA_MAX_SIZE, &_out->response_size, &_out->code ) == HB_FAILURE )
    {
        return HB_FAILURE;
    }
