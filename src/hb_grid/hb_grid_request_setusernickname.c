...
hb_http_code_t hb_grid_request_setusernickname( hb_grid_request_handle_t * _args )
{
    ...
    if( hb_json_get_field_string( _args->data, "nickname", &arg_nickname, HB_NULLPTR ) == HB_FAILURE )
    {
        snprintf( _args->reason, HB_GRID_REASON_DATA_MAX_SIZE, "invalid get nickname" );
