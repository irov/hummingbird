...
hb_http_code_t hb_grid_request_command( struct evhttp_request * _request, hb_grid_process_handle_t * _process, char * _response, hb_size_t * _size, const hb_grid_process_cmd_args_t * _args )
{
    const char * arg_account_token = _args->arg1;
    const char * arg_puid = _args->arg2;
    const char * arg_method = _args->arg3;
    ...
    in_data.puid = puid;
    in_data.uuid = HB_UID_NONE;

    strcpy( in_data.api, "command" );
