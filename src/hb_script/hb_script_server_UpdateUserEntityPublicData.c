...
int __hb_script_server_UpdateUserEntityPublicData( lua_State * L )
{
    ...
    if( hb_db_get_values( g_script_handle->db_collection_user_entities, eoid, db_fields, user_entity_values, 1 ) == HB_FAILURE )
    {
        HB_SCRIPT_ERROR( L, "internal error" );
    }
    ...
    if( hb_db_update_values( g_script_handle->db_collection_user_entities, eoid, update_handler, 1 ) == HB_FAILURE )
    {
        HB_SCRIPT_ERROR( L, "internal error" );
    }
