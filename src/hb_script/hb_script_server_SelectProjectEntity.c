...
#define HB_SCRIPT_SELECT_PROJECT_ENTITY_MAX 512
#endif
...
int hb_script_server_SelectProjectEntity( lua_State * L )
{
    ...
    if( hb_db_select_values( script_handle->db_collection_project_entities, query, db_fields, 1, values, HB_SCRIPT_SELECT_PROJECT_ENTITY_MAX, &exists ) == HB_FAILURE )
    {
        HB_SCRIPT_ERROR_INTERNAL_ERROR( L );
    }
